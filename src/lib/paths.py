...
def check_paths() -> bool:
    ...
    if not PATHS["U_VISION_TEMPLATES_PATH"].exists():
        # find the correct drive by searching
        # for a drive with the folder `t_lernende` in it
        ...
        for letter in drive_letters:
            ...
            if path.exists():
                ...
                for key in PATHS.keys():
                    ...
                    if wrong_drive == "T:":
                        PATHS[key] = (
                            Path(f"{letter}\\t_lernende")
                            / str(PATHS[key]).split("\\", 1)[1]
                        )
                    elif wrong_drive == "N:":
                        PATHS[key] = (
                            Path(f"{letter}\\n_home-s")
                        ...
                        PATHS[key] = (
                            Path(f"{letter}\\s_mitarbeiter")
                            / str(PATHS[key]).split("\\", 1)[1]
                        )
                    elif wrong_drive == "U:":
                        PATHS[key] = (
                            Path(f"{letter}\\u_archiv")
                            / str(PATHS[key]).split("\\", 1)[1]
                        )
